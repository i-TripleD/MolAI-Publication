...
for i in range(1):
    ...
    for train, test in kf.split(x_train, y_train):     #train
        ...
        history = DNN_model.fit(x_train[train], y_train[train], epochs=100, validation_data=(x_train[test], y_train[test]), callbacks=[early_stopping, lr_scheduler], verbose=1, batch_size = 16)
