...
train_dataset_bbb = train.drop("Target",axis=1)
train_labels_bbb = train.pop('Target')   
...
class_counts = dict(Counter(train_labels_bbb))

minority_class_counts = {c: max(30000, n_samples) for c, n_samples in class_counts.items() if n_samples < 30000}
majority_class_counts = {c: min(30000, n_samples) for c, n_samples in class_counts.items() if n_samples > 30000}

# Create an instance of RandomUnderSampler and SMOTETomek
rus = RandomUnderSampler(sampling_strategy=majority_class_counts, random_state=42)
smt = SMOTETomek(sampling_strategy=minority_class_counts, smote=SMOTE(k_neighbors=5), n_jobs=-1)

# Apply random undersampling to the majority class
x_train_resampled1, y_train_resampled1 = rus.fit_resample(train_dataset_bbb, train_labels_bbb)

# Apply SMOTE and Tomek links to the resampled dataset
x_train_resampled2, y_train_resampled2 = smt.fit_resample(x_train_resampled1, y_train_resampled1)

# Print the number of samples in each class after resampling
print(Counter(y_train_resampled2))

x_train = x_train_resampled2.to_numpy()
y_train = y_train_resampled2.to_numpy()
...
for i in range(1):
    ...
    for train, test in kf.split(x_train, y_train):     #train
        ...
        history = DNN_model.fit(x_train[train], y_train[train], epochs=100, validation_data=(x_train[test], y_train[test]), 
                                callbacks=[early_stopping, lr_scheduler], verbose=1, batch_size = 32)
